...
export const gsawPlugin: Plugin = {
  name: 'plugin-starter',
      ...
      tests: [
        ...
        },
        {
          ...
          fn: async (runtime) => {
            ...
            const actionExists = gsawPlugin.actions.some((a) => a.name === 'HELLO_WORLD');
            if (!actionExists) {
...
// Add debugging info to help understand why tests aren't running
{
  const debugPlugin = () => {
    ...
    logger.debug('Plugin name:', gsawPlugin.name);
    logger.debug('Tests array exists:', !!gsawPlugin.tests);
    logger.debug('Tests array length:', gsawPlugin.tests?.length);
    if (gsawPlugin.tests && gsawPlugin.tests.length > 0) {
      logger.debug('First test suite name:', gsawPlugin.tests[0].name);
      logger.debug('First test suite has tests array:', !!gsawPlugin.tests[0].tests);
      logger.debug('First test suite tests length:', gsawPlugin.tests[0].tests?.length);
      if (gsawPlugin.tests[0].tests && gsawPlugin.tests[0].tests.length > 0) {
        logger.debug('First test name:', gsawPlugin.tests[0].tests[0].name);
        logger.debug('First test has fn:', !!gsawPlugin.tests[0].tests[0].fn);
      }
    }
  };
